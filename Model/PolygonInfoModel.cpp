...
QVariant PolygonInfoModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (role == Qt::DisplayRole && orientation == Qt::Horizontal) {
        QStringList headers = {"ID", "Точек до", "Точек после", "Время (мс)", "Макс. отклонение"};
...
int PolygonInfoModel::columnCount(const QModelIndex &parent) const
{
    ...
    return 5; // ID, pointsBefore, pointsAfter, timeMs, maxDeviation
}
...
QVariant PolygonInfoModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        case 3: // Time
            return info.isSimplified ? QString::number(info.timeMs) + " ms" : QString("—");
...
void PolygonInfoModel::updatePolygonAfterSimplification(int id, int pointsAfter, qint64 timeMs, double maxDeviation)
{
    if (id >= 0 && id < m_polygonInfos.size()) {
        m_polygonInfos[id].pointsAfter = pointsAfter;
        m_polygonInfos[id].timeMs = timeMs;
