...
double CoordinateConverter::degToRad(double deg) { return deg * M_PI / 180.0; }
...
double CoordinateConverter::lonDifferenceInMeters(double lat1, double lat2, double lon1,
                                                  double lon2) {
  double R = EQUATORIAL_EARTH_RADIUS_METERS;
...
QList<PolygonPair> CoordinateConverter::convertLatLonToMetersList(
    const QList<PolygonPair>& latLonList, double minLon, double minLat) {
...
QPointF CoordinateConverter::getCornerInMeters(double minLon, double maxLon, double minLat,
                                               double maxLat) {
...
double CoordinateConverter::distanceBetweenLatLonPoints(double lat1, double lon1, double lat2,
                                                        double lon2) {
  double R = EQUATORIAL_EARTH_RADIUS_METERS;
